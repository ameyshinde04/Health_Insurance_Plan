...
const Compare: React.FC = () => {
  ...
  const selectedPlans = SAMPLE_DATA.filter(p => selectedPlanIds.includes(p.PlanId));
  ...
  const getMetalBadgeClasses = (level: string) => {
    ...
    switch (normalized) {
      ...
      case 'Platinum':
        return 'bg-slate-800 text-white border border-slate-900 shadow-sm';
  ...
  const comparisonRows = [
    ...
    { label: 'Family (Per Person) Ded.', key: 'TEHBDedInnTier1FamilyPerPerson' },
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <header className="mb-12">
        ...
        <p className="text-slate-500">Select up to 4 plans to analyze technical specifications side-by-side.</p>
      </header>

      {/* Plan Selection Bar */}
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-12 flex flex-wrap gap-4 items-center">
        <span className="text-sm font-bold text-slate-400 uppercase tracking-widest mr-4">Quick Add:</span>
        {SAMPLE_DATA.map(plan => (
          <button
            key={plan.PlanId}
          ...
          >
            {selectedPlanIds.includes(plan.PlanId) ? <X className="inline w-3 h-3 mr-1" /> : <Plus className="inline w-3 h-3 mr-1" />}
      ...
      {selectedPlans.length === 0 ? (
        <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl p-24 text-center">
          ...
          <h2 className="text-xl font-bold text-slate-400">No comparison selected</h2>
          <p className="text-slate-400 mt-2">Click plans above to build your comparison table.</p>
        </div>
      ) : (
            ...
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="px-8 py-6 text-left w-64 sticky left-0 bg-slate-50 z-10">
                  <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Plan Details</span>
                </th>
                {selectedPlans.map(plan => (
                  <th key={plan.PlanId} className="px-8 py-6 text-left min-w-[200px]">
                    <div className="flex justify-between items-start mb-2">
                      <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider ${getMetalBadgeClasses(plan.MetalLevel)}`}>
                        {plan.MetalLevel}
                      </span>
                      <button onClick={() => togglePlan(plan.PlanId)} className="text-slate-400 hover:text-red-500 p-1">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                    <p className="text-sm font-bold text-slate-900 leading-tight">{plan.PlanVariantMarketingName}</p>
                    <p className="text-[10px] text-slate-400 font-mono mt-1">{plan.PlanId}</p>
                  </th>
                ))}
              </tr>
            </thead>
              ...
              {comparisonRows.map((row) => (
                <tr key={row.key} className="group hover:bg-slate-50 transition-colors">
                  ...
                  {selectedPlans.map(plan => {
                    ...
                    return (
                      <td key={`${plan.PlanId}-${row.key}`} className="px-8 py-5 text-sm text-slate-800 font-medium">
                        {value === 'Yes' ? (
